...
void proc_init(pcb_t *proc) {
    ...
    memset((mem + (freeFrame * PAGE_SIZE)), 0x0, PAGE_SIZE);
    /*
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    pte_t *curr_table_entry = (pte_t * )(mem + (PTBR * PAGE_SIZE)) + curr_vpn;
    ...
    if (!(curr_table_entry->valid)) {
        page_fault(address);
    }

    /* Update the timestamp of the appropriate frame table entry with the provided get_current_timestamp function. */
    frame_table[curr_table_entry->pfn].timestamp = get_current_timestamp();
    frame_table[curr_table_entry->pfn].referenced = 1;
    /*
    ...
    */
    uint16_t ofst = vaddr_offset(address);
    paddr_t phys_addr = (paddr_t)(((curr_table_entry->pfn) << OFFSET_LEN) | ofst);
...
void proc_cleanup(pcb_t *proc) {
    ...
    pfn_t curr_pfn = proc->saved_ptbr;
    pte_t *page_table = (pte_t * )(mem + (curr_pfn * PAGE_SIZE));
    /* Iterate the page table and clean up each valid page */
    for (size_t i = 0; i < NUM_PAGES; i++) {
        pte_t *curr_pte = i + page_table;
        if (curr_pte->valid == 1) {
            pfn_t pfn_i = curr_pte->pfn;
            fte_t *ft_i = (fte_t * )(mem) + pfn_i;
            ft_i->protected = 0;
            ft_i->mapped = 0;
    ...
    frame_table[curr_pfn].protected = 0;
    frame_table[curr_pfn].mapped = 0;
}
